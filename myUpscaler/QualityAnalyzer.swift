...
final class QualityAnalyzer {
    ...
    // MARK: - Preferred API (macOS 13+): async
    func analyzeAsync(inputPath: String, maxSamples: Int = 12) async -> QualityAnalysis {
        guard FileManager.default.fileExists(atPath: inputPath) else {
            return QualityAnalysis(
                metrics: QualityMetrics(noise: 0, blur: 0, blockiness: 0, banding: 0),
                notes: ["Quality scan skipped (file missing)."]
            )
        }
        ...
        do {
            ...
            guard let first = tracks.first else {
                return QualityAnalysis(
                    metrics: QualityMetrics(noise: 0, blur: 0, blockiness: 0, banding: 0),
                    notes: ["Quality scan skipped (no video track)."]
                )
            }
            track = first
        } catch {
            return QualityAnalysis(
                metrics: QualityMetrics(noise: 0, blur: 0, blockiness: 0, banding: 0),
                notes: ["Quality scan skipped (failed to load tracks)."]
            )
        }
        ...
        do {
            ...
            return QualityAnalysis(
                metrics: QualityMetrics(noise: 0, blur: 0, blockiness: 0, banding: 0),
                notes: ["Quality scan skipped (reader init failed)."]
            )
        }
        ...
        guard reader.startReading() else {
            return QualityAnalysis(
                metrics: QualityMetrics(noise: 0, blur: 0, blockiness: 0, banding: 0),
                notes: ["Quality scan skipped (reader failed)."]
            )
        }
        
        // macOS 13+: nominalFrameRate is async-loaded
        let fps: Float = (try? await track.load(.nominalFrameRate)) ?? 30
        let sampleStride = max(1, Int(max(1, Double(fps)).rounded()) / 3)
        ...
        guard !stats.isEmpty else {
            return QualityAnalysis(
                metrics: QualityMetrics(noise: 0, blur: 0, blockiness: 0, banding: 0),
                notes: ["Quality scan skipped (no frames read)."]
            )
        }
    ...
    // MARK: - Backwards-compatible sync wrapper (blocks current thread)
    // Prefer switching your callers to analyzeAsync instead of using this.
    func analyze(inputPath: String, maxSamples: Int = 12) -> QualityAnalysis {
        let sema = DispatchSemaphore(value: 0)
        var out = QualityAnalysis(
            metrics: QualityMetrics(noise: 0, blur: 0, blockiness: 0, banding: 0),
            notes: ["Quality scan skipped (internal)."]
        )
        
        Task {
            out = await analyzeAsync(inputPath: inputPath, maxSamples: maxSamples)
            sema.signal()
        }
